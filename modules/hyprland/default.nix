{ config, pkgs, ... }:
{
  home-manager.users.${config.my.user.name}.wayland.windowManager.hyprland = {
  ...
  home-manager.users.${config.my.user.name}.programs.wofi.enable = true;
  home-manager.users.${config.my.user.name}.services.hyprpaper = {
    enable = true;
    settings = {
  ...
  home-manager.users.${config.my.user.name}.gtk = {
    enable = true;
  ...
  home-manager.users.${config.my.user.name}.home.pointerCursor = {
    gtk.enable = true; x11.enable = true; hyprcursor.enable = true;
    package = pkgs.rose-pine-cursor; name = "BreezeX-RosePine-Linux";
  };
  home-manager.users.${config.my.user.name}.programs.kitty = { enable = true; settings.confirm_os_window_close = 0; };
}
